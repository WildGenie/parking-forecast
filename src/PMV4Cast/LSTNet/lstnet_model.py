...
def LSTNetModel(init, input_shape):
	...
	# Apply a Conv2D that will transform it into data of dimensions (batchsize, time, 1, NumofFilters)
        ...
        C = Dropout(init.dropout)(C, training=True)
    ...
    _, R = GRU(int(init.GRUUnits), activation="relu", return_sequences = False, return_state = True)(C)
    R    = Dropout(init.dropout)(R, training=True)
    ...
    # Dense layer
    Y = Flatten()(R)
    Y = Dropout(init.dropout)(Y, training=True) ################################## added
    Y = Dense(m)(Y)
    ...
    if init.highway > 0:
        ...
        Z = Flatten()(Z)
        Z = Dropout(init.dropout)(Z, training=True) ################################## added
        Z = Dense(1)(Z)
        Z = PostARTrans(m)([Z,X])
...
# - nd:          A metrics function that calculates the normalized deviation                                          #
# - nrmse:       A metrics function that calculates the normalized root mean square error                             #
# - rho_risk:    A metrics function that calculates the quantile risk                                                 #
#######################################################################################################################
...
def nd(y_true, y_pred):
    #
    # This function calculates the normalized deviation between the true and the predicted outputs
    #

    num = K.sum(K.abs(y_true - y_pred), axis=None)
    den = K.sum(K.abs(y_true), axis=None)

    return num / den #K.mean(num / den)


## rho risk per variable ; if axis =1 thean per timestamp
...
def corr(y_true, y_pred):
    ...
    #
    num1 = y_true - K.mean(y_true, axis=None)
    num2 = y_pred - K.mean(y_pred, axis=None)

    num  = K.mean(num1 * num2, axis=None)
    den  = K.std(y_true, axis=None) * K.std(y_pred, axis=None)
...
def ModelCompile(model, init):
    ...
    model.compile(optimizer = opt, loss = init.loss, metrics=[rse, corr, nrmse, nd])
